...
| --- | --- | --- |
| Classes | `PascalCase` | `KinematicStateSnapshot`, `RollController` |
...
```text
src/
  laf/                      # top-level package (liteaero flight)
    __init__.py
    units/
      __init__.py
...
Every dynamic flight software component implements this abstract base class:
...
class Component(ABC):
    """Base interface for all dynamic flight software components."""
...
- Unit conversions are isolated in `laf/units/conversion.py`.
- Never call unit conversion functions inside physics or control computation code.

```python
# laf/units/conversion.py — the SI boundary
import math
...
from laf.units.conversion import deg_to_rad
...
class Integrator(Component):
    ...
    def deserialize(self, state: dict[str, Any]) -> None:
        ...
        if version != SCHEMA_VERSION:
            raise ValueError(f"Integrator: unsupported schema version {version}")
        self._output = state["output_"]
        self._dt_s   = state["dt_s_"]

    def to_json(self) -> str:
        return json.dumps(self.serialize(), indent=2)

    @classmethod
    def from_json(cls, json_str: str) -> "Integrator":
        obj = cls()
...
```python
# test/control/test_integrator.py

import pytest
from laf.control.integrator import Integrator

SCHEMA_VERSION = 1

@pytest.fixture
def default_config() -> dict:
    return {"dt_s": 0.01, "initial_value": 0.0}

@pytest.fixture
def integrator(default_config) -> Integrator:
    obj = Integrator()
    obj.initialize(default_config)
    return obj


class TestIntegrator:

    def test_zero_input_produces_zero_output(self, integrator: Integrator) -> None:
        integrator.step(0.0)
        assert integrator.out() == pytest.approx(0.0, abs=1e-9)

    def test_serialize_deserialize_round_trip(self, integrator: Integrator, default_config) -> None:
        integrator.step(1.0)
        snapshot = integrator.serialize()

        restored = Integrator()
        restored.initialize(default_config)
        restored.deserialize(snapshot)

        assert integrator.serialize() == restored.serialize()

    def test_deserialize_rejects_unknown_schema_version(self, integrator: Integrator) -> None:
        bad_state = integrator.serialize()
        bad_state["schema_version"] = 999
        with pytest.raises(ValueError, match="schema version"):
            integrator.deserialize(bad_state)
```
...
- Run tests with `pytest --cov=laf --cov-report=term-missing`.
...
Python version is pinned in `python/.python-version`. Dependencies are declared in
`python/pyproject.toml`. This directory and tooling are created when Python is first
...
- All production logic lives in `src/laf/`, not in notebooks.
- Notebooks import from `laf` like any other consumer.
